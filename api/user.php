...
switch($method)
{
   case "GET": 
    ...
    if(isset($path[4]) && is_numeric($path[4]))
    {
      ...
      {
       $json_array['rowUserdata']= array('id'=>$userrow['userid'],'username'=>$userrow['username'], 'email'=>$userrow['useremail'],);
      }
    ...
    if(mysqli_num_rows($alluser) > 0)
    {
      ...
      {
       $json_array["userdata"][]= array("id"=>$row['userid'], "username"=>$row["username"], "email"=>$row["useremail"]);
      }
    ...
    case "POST":
        ...
        $result= mysqli_query($db_conn, "INSERT INTO tbl_user (username, useremail) 
        ...
        break;
         ...
         case "PUT":
          $userUpdate= json_decode(file_get_contents("php://input"));
           ...
           $updateData= mysqli_query($db_conn, "UPDATE tbl_user SET username='$username', useremail='$useremail' WHERE userid='$userid'  ");
           if($updateData)
           {
